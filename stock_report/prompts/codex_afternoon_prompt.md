...
在 GitHub 仓库 `ldwghm/AI-Mark-information` 的 `main` 分支执行 `stock_report/prompts/afternoon_prompt.md` 的完整生产 playbook。

约束：

- 使用已连接的 GitHub 身份，不在日志或提交中输出凭据。
- 只产出 `afternoon_latest.json` 和 `afternoon_analysis_candidate.json`；邮件、最终 analysis 和归档全部交给 GitHub Actions。
